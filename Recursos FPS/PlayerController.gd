...
func _ready():
	...
	# Establecemos la posición inicial del arma
...
func weapon_sway(delta):
	...
	weapon_holder.rotation.x = lerp(weapon_holder.rotation.x, mouse_input.y * weapon_rotation_amount, delta)
	weapon_holder.rotation.y = lerp(weapon_holder.rotation.y, mouse_input.x * weapon_rotation_amount, delta)
	...
	if weapon_holder:
		if vel > 0:
			weapon_holder.position.y = lerp(
				weapon_holder.position.y, def_weapon_holder_pos.y + sin(Time.get_ticks_msec() * wobble_freq) * wobble_amount, 10 * delta
			)
			weapon_holder.position.x = lerp(
				weapon_holder.position.x,
				def_weapon_holder_pos.x + sin(Time.get_ticks_msec() * wobble_freq * 0.5) * wobble_amount,
				10 * delta
			)
		else:
